...
- **BrowserToolbar Filter Clear Icon Overlay Fix**: Fixed an overlay bug where the clear filter button ("X") in `BrowserToolbar.vue` rendered directly on top of long filter text (e.g., `sickOllie_krea2_int8`). Updated `primevue-overrides.css` with dynamic right-padding (`padding-right: 44px` when `.p-dropdown-clear-icon` is present vs `26px` default), added `.p-dropdown-clear-icon` absolute vertical centering (`right: 24px`), and added `<template #clearicon>` with Lucide `<X :size="12" />` to all toolbar filter dropdowns.
- **Verification**: Executed backend unit suite (`cd backend && ./mvnw test` — 154/154 tests passed) and frontend production build (`cd frontend && npm run build` — 1,946 modules transformed, built clean in 2.19s).
