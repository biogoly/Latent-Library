...
- **BrowserToolbar Filter Clear Icon Overlay, Click Propagation & Label Flex Fill Fix**: Resolved the root cause of prematurely cropped filter labels in `BrowserToolbar.vue`. PrimeVue applies `.p-inputtext` to `.p-dropdown-label`, which caused global `.p-inputtext` rules to inject a nested inner box (`border`, `background: #23252F`, `padding: 6px 12px`) inside `.p-dropdown`, shrinking the label text into a tiny box on the left (`z_ima`, `DPM+`, `Illustr`). Updated `primevue-overrides.css` and `BrowserToolbar.vue` to override `.p-dropdown-label` with `background: transparent; border: none; flex: 1 1 auto; width: 1%;`, eliminating the nested box so label text expands smoothly across the full dropdown width up to `right: 34px`.
- **Verification**: Executed backend unit suite (`cd backend && ./mvnw test` — 154/154 tests passed) and frontend production build (`cd frontend && npm run build` — 1,946 modules transformed, built clean in 1.99s).
